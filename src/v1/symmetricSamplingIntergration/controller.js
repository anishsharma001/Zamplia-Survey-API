...
const { getLangIdFromDb, upsertStudiesData, getAllLiveSurveyFromDb, getAllQualificationFromDb, getAllOptionsFromDb, pauseLucidSurveys } = require('./dao');
const { createSuveyBundle, insertQuals, insertQuotas } = require('./operations.js')
const {difference} = require('lodash');

async function getProjectFromApi(lang_Code) {
    try {
        let surveysFromSymmtric = await NetworkService.getAllSurveys();
        let surveys = [];
        

        let LanguageId = await getLangIdFromDb(lang_Code);
        if (surveysFromSymmtric.success && surveysFromSymmtric.result.Surveys.length) {
            surveys = surveysFromSymmtric.result.Surveys;
            ...
            if (surveysToPause.length) {
                ...
                pauseLucidSurveys(surveysToPause, 35);
            }
        }
        return
    } catch (error) {
...
module.exports = {
    getProjectFromApi: getProjectFromApi
};