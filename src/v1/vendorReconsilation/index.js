...
async function insertVendorReconsilation(req, res) {
    try {
        ...
        const vendorCategorySummaryQuery = `
            SELECT 
                ...
                PositiveReconciliation, NegativeReconciliation, calibar8_score, manualCalibr8Score, created_at, updatedAt, currentMonthRecon, lastMonthRecon, olderMonthRecon, threshold
...
async function getCalibr8ScoreByGroup(vendors, start, end) {
    try {
        ...
        for (let v = 0; v < vendorChunks.length; v++) {
            ...
            for (let w = 0; w < weeklyChunks.length; w++) {
                ...
                const query = `
                    ...
                    WHERE cs.created_at >= '${chunkStart}'
        ...
        console.log(`Final results: ${results.length} vendor combinations`);
        return results;
