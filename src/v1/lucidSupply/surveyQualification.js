    ...
    for (const questionIndex of allQuestions) {
      ...
      if (Number(questionIndex.QuestionID) === 42 || Number(questionIndex.QuestionID) === 45 || postalCodes.includes(QuestionId)) {
        if (Number(questionIndex.QuestionID) === 45 || postalCodes.includes(QuestionId)) {
          ...
          bundleNonRangeQualification.push([sqid, surveyId, filterQuestion.demographicId, filterQuestion._id, "", optionsText, date, date, lang_code, sqid]);
          ...
          //   lang_code
          // ])
          bundleRangeOrder.push([surveyId, filterQuestion.demographicId, screenerOrderNumber, date, date, lang_code, sqid]);
        } else {
          const ages = questionIndex.PreCodes;
          const minAge = Math.min(...ages.map(Number));
          const maxAge = Math.max(...ages.map(Number));
          bundleRangeQualification.push([sqid, surveyId, filterQuestion.demographicId, filterQuestion._id, maxAge, minAge, date, date, lang_code, sqid]);
          // bundleGlobalDemoData.push([
          //   sqid,
          //   surveyId,
          //   filterQuestion.demographicId,
        ...
        const filterOption = allDbOptions.filter((obj1) => questionIndex.PreCodes.some((obj2) => obj2 == obj1.lucidOpId && obj1.queryId === filterQuestion._id));
        const optionsIds = filterOption.map((d) => d._id).join(",");
        bundleNonRangeQualification.push([sqid, surveyId, filterQuestion.demographicId, filterQuestion._id, optionsIds, "", date, date, lang_code, sqid]);
        // bundleGlobalDemoData.push([
        //   sqid,
        //   surveyId,
        //   filterQuestion.demographicId,
